...
### Инвентаризация существующего сервера

Если сайт разворачивается на сервер, где уже работают другие проекты, соберите
картину окружения до начала работ:

```bash
scp scripts/server-inventory.sh user@сервер:/tmp/
ssh user@сервер 'sudo bash /tmp/server-inventory.sh'
```

Скрипт **только читает** и складывает отчёт в `/tmp/greymouse-server-report-*.txt`:
версии Node/Docker/PostgreSQL, занятые порты, контейнеры и тома, vhost'ы
обратного прокси, базы и роли PostgreSQL, TLS-сертификаты, firewall, текущие
DNS-записи домена. В конце — сводка «что есть, чего не хватает».

Ничего не устанавливает и не меняет. Содержимое `.env`, приватные ключи и
пароли не читаются: из конфигов nginx выводятся только строки маршрутизации,
а учётные данные внутри URL маскируются. Отчёт всё равно стоит просмотреть
перед пересылкой.
